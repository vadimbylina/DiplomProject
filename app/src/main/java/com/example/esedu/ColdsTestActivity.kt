...
class ColdsTestActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cursor = db.rawQuery("SELECT composition FROM colds", null)
    ...
    private fun displayQuestion() {
        ...
        val randomSoupQuery = "SELECT name, composition FROM colds WHERE name NOT IN (${usedSoupNames.joinToString(",") { "'$it'" }}) ORDER BY RANDOM() LIMIT 1"
        ...
        if (randomSoupCursor.moveToFirst()) {
            ...
            if (soupCompositionIndex != -1) {
                ...
                val wrongAnswersQuery =
                    "SELECT composition FROM colds WHERE name != ? ORDER BY RANDOM() LIMIT 3"
    ...
    private fun checkAnswer() {
        ...
        if (selectedRadioButtonId != -1) {
            ...
            val query = "SELECT composition FROM colds WHERE name = ?"
