...
class DrinksTestActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cursor = db.rawQuery("SELECT composition FROM drinks", null)
    ...
    private fun displayQuestion() {
        ...
        val randomSoupQuery = "SELECT name, composition FROM drinks WHERE name NOT IN (${usedSoupNames.joinToString(",") { "'$it'" }}) ORDER BY RANDOM() LIMIT 1"
        ...
        if (randomSoupCursor.moveToFirst()) {
            ...
            if (soupCompositionIndex != -1) {
                ...
                val wrongAnswersQuery =
                    "SELECT composition FROM drinks WHERE name != ? ORDER BY RANDOM() LIMIT 3"
    ...
    private fun checkAnswer() {
        ...
        if (selectedRadioButtonId != -1) {
            ...
            val query = "SELECT composition FROM drinks WHERE name = ?"
    ...
    private fun showResult() {
        ...
        /*  if (correctAnswersCount == cursor.count) {
              ...
              val userId = getUserIdByLogin(MainActivity.login)

              if (userId != null) {
                  val db = DBHelper(this, null)
                  val values = ContentValues().apply {
                      put("userID", userId)
                      put("endingSoupTest", currentDate)
                  }
